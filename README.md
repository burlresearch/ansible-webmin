...
![Robots Building Robots](http://aboutfacts.net/ComputRobot/CR74/PD/Advanced_Automation_for_Space_Missions_figure_5-29.jpg)

## Robots Building Robots

# Introduction

[Ansible][8] is a powerful tool for provisioning [CentOS][1] on commercial grade linux webservers.  In my last article, [Webmin Configuration on CentOS][4], I discussed the steps to setup a new server complete with [Webmin][3] and [Percona][9].  This is our preferred setup for reliable webservers, but one glance at the article will make it obvious that there are quite a few steps.  Like any good programmer, I can't stand repetition, hence seek to automate.
...
We find [Ansible][8] to be the simplest and therefore, most appealing for our purposes.  Other approaches that we have used, like [Chef][11], are very powerful, but equally configuration heavy - and more complex than we typically need for webservers.

Since my last article there is a new minor-version of CentOS available - [CentOS 6.5][1].  This new version alleviates the need for some of the caveats in my last article, but creates some new [*gotcha's*][gotcha] along the way.
...
> `pro-tip:` we present the Ansible scripts first, [you can follow along on your own DigitalOcean droplet][digitalocean]
...
> `pro-tip:` [see how to setup ansible on a workstation][ansible]
...
You will have to clone the `github repo` to see the playbook code.  In the interest of pace, we have decided not to dissect the code here, hopefully you will find Ansible readable enough to self-document.  As [we explain below][ansible], it is simply a matter of checking out the above playbook and running it:
...
I use a Ubuntu Linux workstation - so your mileage may vary [depending on your operating system][15].  For me, setting up Ansible was a matter of:
...
In our next article we will discuss how to put all our developers on equal footing.  We will discuss [*virtualization with Vagrant*][16] - how to precisely mimic a production environment, without any hardware&hellip;
